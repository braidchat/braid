...
(defn run-txns!
  ...
  `:braid.core.server.db/return`, that function will be called with the transaction
  result and the return value added to the seq of return values
  If the metadata map contains a function under the key
  `:braid.core.server.db/check`, that function will be called with the transaction
