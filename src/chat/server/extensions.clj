...
(defmulti handle-oauth-token (fn [ext state code] (ext :type)))
(defmulti handle-thread-change (fn [ext thread-id] (ext :type)))
(defmulti handle-webhook (fn [ext event-req] (ext :type)))
(defmulti extension-config (fn [ext data] (ext :type)))
