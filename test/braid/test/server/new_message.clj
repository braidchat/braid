    ...
    (testing "new messages can create a thread"
      (let [group (group/create-group! db/conn {:id (db/uuid) :name "group"})
        ...
        (testing "user has thread subscribed"
          (is (contains?
                (set (thread/subscribed-thread-ids-for-user db/conn (user-1 :id)))
                thread-id)))))
  ...
  (testing "given a thread with 2 participants"
    (let [user-1 (user/create-user! db/conn {:id (db/uuid)
      ...
      (testing "when user-2 hides the thread"
        (thread/user-hide-thread! db/conn (user-2 :id) thread-id)

        (testing "then user-2 no longer has the thread open"
          (is (not (contains?
                     (set (map :id (thread/open-threads-for-user db/conn (user-2 :id))))
                     thread-id)))))
        ...
        (testing "when a new message mentions the tag..."
          (let [msg (message/create-message! db/conn {:id (db/uuid)
            ...
            (testing "then the tag is added to the thread"
              (let [thread (thread/thread-by-id db/conn (msg :thread-id))]
                (contains? (set (thread :tag-ids)) (tag-1 :id))))

            (testing "then the user has the thread opened"
              (let [user-threads (map :id (thread/open-threads-for-user db/conn (user-1 :id)))]
                (is (contains? (set user-threads) (msg :thread-id)))))

            (testing "then the user is subscribed-to the thread"
              (let [user-threads (thread/subscribed-thread-ids-for-user db/conn (user-1 :id))]
                (is (contains? (set user-threads) (msg :thread-id))))
              (let [users (thread/users-subscribed-to-thread db/conn (msg :thread-id))]
      ...
      (testing "when user-1 mentions user-2 in a message..."
        (let [msg (message/create-message! db/conn {:id (db/uuid)
          ...
          (testing "then user-2 is added to the thread"
            (is  (= #{(user-2 :id)}
                    (-> (thread/thread-by-id db/conn thread-id) :mentioned-ids))))

          (testing "then user-2 can see the thread"
            (is (thread/user-can-see-thread? db/conn (user-2 :id) thread-id)))
          ...
          (testing "then user-2 has the thread opened"
            (is (= [thread-id] (map :id (thread/open-threads-for-user db/conn (user-2 :id)))))))))))
